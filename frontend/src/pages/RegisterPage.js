...
const RegisterPage = () => {
  ...
  const bioOnly = searchParams.get('step') === 'bio'; // Check if this is bio-only mode
  ...
  const handleSubmit = async (e) => {
    ...
    // If bio-only mode, update user bio
    if (bioOnly) {
      if (!formData.bio || formData.bio.trim() === '') {
        const errorMsg = 'Please tell us about yourself';
        setError(errorMsg);
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            {bioOnly ? 'Complete Your Profile' : 'Join Wingman'}
          </h1>
          <p className="text-gray-600">
            {bioOnly ? 'Tell us a bit about yourself to get started' : 'Start your accountability journey today'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? (bioOnly ? 'Saving...' : 'Creating Account...') : (bioOnly ? 'Complete Profile' : 'Create Account')}
          </button>
        </form>
